...
# never be edited/removed through the extension (it is what EverLua is FOR).
...
def _reclaim_bridge_port():
    ...
    if "bridge.py" not in cmdline.lower():
        log(f"port {PORT} is held by pid {pid_i} ('{name}') but it does not look "
            f"like a EverLua bridge - leaving it alone.", "yl")
        return False
    log(f"port {PORT} is held by a leftover EverLua bridge (pid {pid_i}) from a "
        ...
        with self.start_lock:
            ...
            with _Spinner(f"    [{self.id}] starting..."):
                ...
                self._request("initialize", {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "everlua-bridge", "version": "1.0"},
                }, timeout=30)
...
async def main():
    print(f"\n{C['cy']}  EverLua Bridge v{BRIDGE_VERSION}{C['reset']}  {C['dim']}- Roblox Studio - ws://{HOST}:{PORT}{C['reset']}\n")
